    ...
    if (!args[0]) {
        shacks.findOne({ userID: message.author.id }, (err, data) => {
            if (err) {
                ...
                return message.channel.send(`You do not own a shack! Use \`!found\` to found your shop!`)
            } else if (data) {
...
Cost: $${funcs.costCalc(upgrades[231].price, data.employees[231])}
Boost: +$${upgrades[231].boost}/hr
ID: 231\n
**Cook**  \`(${data.employees[232]}/15)\`
Cost: $${funcs.costCalc(upgrades[232].price, data.employees[232])}
Boost: +${upgrades[232].boost}/hr
ID: 232\n
**Sous Chef**  \`(${data.employees[233]}/13)\`
Cost: $${funcs.costCalc(upgrades[233].price, data.employees[233])}
Boost: +${upgrades[233].boost}/hr
ID: 233\n
**Head Chef**  \`(${data.employees[234]}/13)\`
Cost: $${funcs.costCalc(upgrades[234].price, data.employees[234])}
Boost: +${upgrades[234].boost}/hr
ID: 234\n
**Executive Chef**  \`(${data.employees[235]}/12)\`
Cost: $${funcs.costCalc(upgrades[235].price, data.employees[235])}
Boost: +${upgrades[235].boost}/hr
ID: 235\n
**Advertiser**  \`(${data.employees[236]}/13)\`
Cost: $${funcs.costCalc(upgrades[236].price, data.employees[236])}
Boost: +${upgrades[236].boost}/hr
ID: 236\n
**Greeter**  \`(${data.employees[237]}/12)\`
Cost: $${funcs.costCalc(upgrades[237].price, data.employees[237])}
    ...
    if (args[0]) {
        shacks.findOne({ userID: message.author.id }, (err, data) => {
            if (err) {
                ...
                if (id > 120 && id < 127) return message.channel.send(`That is an upgrade! Use \`${prefix}buy [ID]\` to purchase!`)

                var cost = funcs.costCalc(upgrades[id].price, data.employees[id])
